...
/* Resources */
extern coap_resource_t  res_washing_m;
...
/* PROCESS */
PROCESS(er_washing_m_server, "Erbium Washing Machine Server");
AUTOSTART_PROCESSES(&er_washing_m_server);

PROCESS_THREAD(er_washing_m_server, ev, data){
  PROCESS_BEGIN();
  ...
  LOG_INFO("Starting Erbium Washing Machine Server\n");
  ...
  while(!reg){
    ...
    sprintf(reg_payload, "{\"ip\":\"%s\", \"type\":\"%s\",\"uri\":\"%s\",\"operations\":\"%s\", \"post_format\":\"%s\",\"get_format\":\"%s\"}", my_ip_addr, WASH_ACT_TYPE, WASH_ACT_URL, WASH_ACT_OPERATIONS, WASH_ACT_PFORMAT, NONE_FORMAT);
  ...
  coap_activate_resource(&res_washing_m, "washing_m");
  etimer_set(&e_timer, CLOCK_SECOND * 2);

  while(1) {
    PROCESS_WAIT_EVENT();
    if(ev == PROCESS_EVENT_TIMER && data == &e_timer){
      res_washing_m.trigger();
