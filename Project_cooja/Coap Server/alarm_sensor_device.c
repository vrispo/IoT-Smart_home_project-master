...
/* Resources */
extern coap_resource_t  res_alarm_s;
...
/* PROCESS */
PROCESS(er_alarm_s_server, "Erbium Alarm Sensor Server");
AUTOSTART_PROCESSES(&er_alarm_s_server);

PROCESS_THREAD(er_alarm_s_server, ev, data){
  PROCESS_BEGIN();
  ...
  LOG_INFO("Starting Erbium Alarm Sensor Server\n");
  ...
  while(!reg){
    ...
    sprintf(reg_payload, "{\"ip\":\"%s\",\"type\":\"%s\",\"uri\":\"%s\",\"operations\":\"%s\",\"post_format\":\"%s\",\"get_format\":\"%s\"}", my_ip_addr, ALARM_SENS_TYPE, ALARM_SENS_URL, ALARM_SENS_OPERATIONS, NONE_FORMAT, NONE_FORMAT);
  ...
  coap_activate_resource(&res_alarm_s, "alarm_s");
  etimer_set(&e_timer, CLOCK_SECOND * 2);
