...
/* Resources */
extern coap_resource_t  res_light_s;
...
/* PROCESS*/
PROCESS(er_light_s_server, "Erbium Light Server");
AUTOSTART_PROCESSES(&er_light_s_server);

PROCESS_THREAD(er_light_s_server, ev, data){
  PROCESS_BEGIN();
  ...
  LOG_INFO("Starting Erbium Light Server\n");
  ...
  while(!reg){
    ...
    sprintf(reg_payload, "{\"ip\":\"%s\", \"type\":\"%s\",\"uri\":\"%s\",\"operations\":\"%s\", \"post_format\":\"%s\",\"get_format\":\"%s\"}", my_ip_addr, LIGHT_SENS_TYPE, LIGHT_SENS_URL, LIGHT_SENS_OPERATIONS, NONE_FORMAT, NONE_FORMAT);
  ...
  coap_activate_resource(&res_light_s, "light_s");
  etimer_set(&e_timer, CLOCK_SECOND * 2);
