...
/* Resources */
extern coap_resource_t  res_temp_hum;
...
/* PROCESS */
PROCESS(er_temp_hum_server, "Erbium Temperature and Humidity Server");
AUTOSTART_PROCESSES(&er_temp_hum_server);

PROCESS_THREAD(er_temp_hum_server, ev, data){
  PROCESS_BEGIN();
  ...
  LOG_INFO("Starting Erbium Temperature and Humidity Server\n");
  ...
  while(!reg){
    ...
    sprintf(reg_payload, "{\"ip\":\"%s\", \"type\":\"%s\",\"uri\":\"%s\",\"operations\":\"%s\", \"post_format\":\"%s\",\"get_format\":\"%s\"}", my_ip_addr, TEMP_HUM_TYPE, TEMP_HUM_URL, TEMP_HUM_OPERATIONS, NONE_FORMAT, TEMP_HUM_GFORMAT);
  ...
  coap_activate_resource(&res_temp_hum, "temp_hum");
  etimer_set(&e_timer, CLOCK_SECOND * 2);
